// Cloudflare Worker: команды бота для Goofish-парсера (бренды/цены) + API для парсера
  ...
  if (cmd === "/start") {
    ...
    if (!sub) {
      ...
      await tgApi(env, "sendMessage", {
        ...
        text: `Новый запрос доступа (Goofish): ${username}, chat_id: ${chatId}\nОдобрить: /approve ${chatId}`,
      });
...
export default {
  async scheduled(event, env, ctx) {
    // Триггерим goofish-parser workflow каждые 15 минут
  ...
  async fetch(request, env) {
    ...
    return new Response("goofish-bot-worker running");
  },
};
